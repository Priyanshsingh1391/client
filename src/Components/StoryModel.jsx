...
const StoryModel = ({setShowModal, fetchStories}) => {
  ...
  "#F39C12"  // Amber
    ...
    const MA_VIDEO_SIZE_MB = 50;

    const handleMediaUpload = (e)=>{
        const file = e.target.files?.[0]
        if(file){
            if(file.type.startsWith("video")){
              if(file.size> MA_VIDEO_SIZE_MB*1024*1024){
                toast.error(`video file size cannot exceed ${MA_VIDEO_SIZE_MB} MB`)
                setMedia(null);
                setPreviewUrl(null)
                return
              }
    ...
    const handleCreateStory = async() => {
      ...
      if (media) {
  formData.append('media', media); // ✅ must send the file
}
  ...
  return (
    <div className='fixed inset-0 z-50 min-h-screen bg-black/80 backdrop-blur
        ...
        relative' style={{backgroundColor: background}}>

          {
            mode === 'text' && (
              ...
              text-lg resize-none focus:outline-none' placeholder='Whats on your mind?'
...
<div className='flex mt-4 gap-2'>
          {bgcolor.map((color)=>(
            ...
            style={{background: color}} onClick={()=> setBackground(color)}/>
          ))}
</div>
